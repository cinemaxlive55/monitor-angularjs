import {Component, NgZone, OnDestroy, OnInit,} from '@angular/core';
import {Location} from '@angular/common';
import { Tier2TectonicService} from '../../services/tier2.tectonic.service';
import {COLORS} from '../../constants/colors';
import {ColorBlockComponent} from '../../components/color-block/color-block.component';


declare var jQuery:any;

@Component({
  selector: 'tectonic',
  templateUrl: 'tectonic.template.html',
  providers: [Tier2TectonicService]
})

export class TectonicComponent implements OnDestroy, OnInit {

  public nav:any;



  private data1 = [
    [this.gd(2012, 1, 1), 7], [this.gd(2012, 1, 2), 6], [this.gd(2012, 1, 3), 4], [this.gd(2012, 1, 4), 8],
    [this.gd(2012, 1, 5), 9], [this.gd(2012, 1, 6), 7], [this.gd(2012, 1, 7), 5], [this.gd(2012, 1, 8), 4],
    [this.gd(2012, 1, 9), 7], [this.gd(2012, 1, 10), 8], [this.gd(2012, 1, 11), 9], [this.gd(2012, 1, 12), 6],
    [this.gd(2012, 1, 13), 4], [this.gd(2012, 1, 14), 5], [this.gd(2012, 1, 15), 11], [this.gd(2012, 1, 16), 8],
    [this.gd(2012, 1, 17), 8], [this.gd(2012, 1, 18), 11], [this.gd(2012, 1, 19), 11], [this.gd(2012, 1, 20), 6],
    [this.gd(2012, 1, 21), 6], [this.gd(2012, 1, 22), 8], [this.gd(2012, 1, 23), 11], [this.gd(2012, 1, 24), 13],
    [this.gd(2012, 1, 25), 7], [this.gd(2012, 1, 26), 9], [this.gd(2012, 1, 27), 9], [this.gd(2012, 1, 28), 8],
    [this.gd(2012, 1, 29), 5], [this.gd(2012, 1, 30), 8], [this.gd(2012, 1, 31), 25]
  ];

  private data2 = [
    [this.gd(2012, 1, 1), 800], [this.gd(2012, 1, 2), 500], [this.gd(2012, 1, 3), 600], [this.gd(2012, 1, 4), 700],
    [this.gd(2012, 1, 5), 500], [this.gd(2012, 1, 6), 456], [this.gd(2012, 1, 7), 800], [this.gd(2012, 1, 8), 589],
    [this.gd(2012, 1, 9), 467], [this.gd(2012, 1, 10), 876], [this.gd(2012, 1, 11), 689], [this.gd(2012, 1, 12), 700],
    [this.gd(2012, 1, 13), 500], [this.gd(2012, 1, 14), 600], [this.gd(2012, 1, 15), 700], [this.gd(2012, 1, 16), 786],
    [this.gd(2012, 1, 17), 345], [this.gd(2012, 1, 18), 888], [this.gd(2012, 1, 19), 888], [this.gd(2012, 1, 20), 888],
    [this.gd(2012, 1, 21), 987], [this.gd(2012, 1, 22), 444], [this.gd(2012, 1, 23), 999], [this.gd(2012, 1, 24), 567],
    [this.gd(2012, 1, 25), 786], [this.gd(2012, 1, 26), 666], [this.gd(2012, 1, 27), 888], [this.gd(2012, 1, 28), 900],
    [this.gd(2012, 1, 29), 178], [this.gd(2012, 1, 30), 555], [this.gd(2012, 1, 31), 993]
  ];


  public flotDataset:any = [
    {
      label: "Number of orders",
      data: this.data2,
      color: "#1ab394",
      bars: {
        show: true,
        align: "center",
        barWidth: 24 * 60 * 60 * 600,
        lineWidth: 0
      }

    }, {
      label: "Payments",
      data: this.data1,
      yaxis: 2,
      color: "#1C84C6",
      lines: {
        lineWidth: 1,
        show: true,
        fill: true,
        fillColor: {
          colors: [{
            opacity: 0.2
          }, {
            opacity: 0.4
          }]
        }
      },
      splines: {
        show: false,
        tension: 0.6,
        lineWidth: 1,
        fill: 0.1
      },
    }
  ];

  public flotOptions:any =
    {
      xaxis: {
        mode: "time",
        tickSize: [3, "day"],
        tickLength: 0,
        axisLabel: "Date",
        axisLabelUseCanvas: true,
        axisLabelFontSizePixels: 12,
        axisLabelFontFamily: 'Arial',
        axisLabelPadding: 10,
        color: "#d5d5d5"
      },
      yaxes: [{
        position: "left",
        max: 1070,
        color: "#d5d5d5",
        axisLabelUseCanvas: true,
        axisLabelFontSizePixels: 12,
        axisLabelFontFamily: 'Arial',
        axisLabelPadding: 3
      }, {
        position: "right",
        clolor: "#d5d5d5",
        axisLabelUseCanvas: true,
        axisLabelFontSizePixels: 12,
        axisLabelFontFamily: ' Arial',
        axisLabelPadding: 67
      }
      ],
      legend: {
        noColumns: 1,
        labelBoxBorderColor: "#000000",
        position: "nw"
      },
      grid: {
        hoverable: false,
        borderWidth: 0
      }
    };

  clusterStatusObj: any = {};
  schedulerNumberObj: any = {};
  crushloopingPodsObj: any = {};
  apiServersObj: any = {};
  controllerManagersObj: any = {};
  alertNumObj: any = {};
  nodeNotReadyObj: any = {};
  podsPercentageObj: any = {};

  public constructor(private tier2TectonicService: Tier2TectonicService, private location: Location, private zone: NgZone) {
    this.nav = document.querySelector('nav.navbar');

    this.clusterStatusObj.up = 'INIT';
  }

  public ngOnInit():any {
    this.nav.className += " white-bg";
    this.tier2TectonicService.getClusterStatus().subscribe({
      next: data => {
        this.zone.run(() => {
          if(data){
            this.clusterStatusObj.up = 'UP';
          }else{
            this.clusterStatusObj.up = 'DOWN';
          }
        });
        console.log(`ClusterStatus up status ${data} get!`);
      },
      error: err => console.error('something wrong occurred: ' + err),
      complete: () => console.log('done'),
    });


    this.tier2TectonicService.getSchedulerNumber().subscribe({
      next: data => {
        this.zone.run(() => {
            this.schedulerNumberObj.num = data;
        });
        console.log(`Scheduler Number Percentage  ${data} get!`);
      },
      error: err => console.error('something wrong occurred: ' + err),
      complete: () => console.log('done'),
    });

    this.tier2TectonicService.getCrushLoopingPods().subscribe({
      next: data => {
        this.zone.run(() => {
          this.crushloopingPodsObj.num = data;
        });
        console.log(`Crushlooping Pods number  ${data} get!`);
      },
      error: err => console.error('something wrong occurred: ' + err),
      complete: () => console.log('done'),
    });

    this.tier2TectonicService.getApiServers().subscribe({
      next: data => {
        this.zone.run(() => {
          this.apiServersObj.num = data;
        });
        console.log(`api servers number  ${data} get!`);
      },
      error: err => console.error('something wrong occurred: ' + err),
      complete: () => console.log('done'),
    });

    this.tier2TectonicService.getControllerManagers().subscribe({
      next: data => {
        this.zone.run(() => {
          this.controllerManagersObj.num = data;
        });
        console.log(`controller managers number  ${data} get!`);
      },
      error: err => console.error('something wrong occurred: ' + err),
      complete: () => console.log('done'),
    });

    this.tier2TectonicService.getAlertNum().subscribe({
      next: data => {
        this.zone.run(() => {
          this.alertNumObj.num = data;
        });
        console.log(`alert number  ${data} get!`);
      },
      error: err => console.error('something wrong occurred: ' + err),
      complete: () => console.log('done'),
    });

    this.tier2TectonicService.getNodeNotReady().subscribe({
      next: data => {
        this.zone.run(() => {
          this.nodeNotReadyObj.num = data;
        });
        console.log(`Node node ready number  ${data} get!`);
      },
      error: err => console.error('something wrong occurred: ' + err),
      complete: () => console.log('done'),
    });

    this.tier2TectonicService.getPodsPercentage().subscribe({
      next: data => {
        this.zone.run(() => {
          this.podsPercentageObj.num = data;
        });
        console.log(`pods percentage number  ${data} get!`);
      },
      error: err => console.error('something wrong occurred: ' + err),
      complete: () => console.log('done'),
    });
  }


  public ngOnDestroy():any {
    this.nav.classList.remove("white-bg");
  }

  private gd(year, month, day) {
    return new Date(year, month - 1, day).getTime();
  }

  goBack(): void {
    this.location.back();
  }

}
